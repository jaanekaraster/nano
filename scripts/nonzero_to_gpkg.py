...
def main():
    ...
    # MapLibre/Mapbox only accept EPSG:4326 for GeoJSON.
    # Always reproject to WGS84 when writing GeoJSON.
    if driver == "GeoJSON":
        from pyproj import Transformer
        from shapely.ops import transform as shp_transform

        source_crs = fiona.crs.CRS.from_wkt(crs_wkt)
        target_crs = fiona.crs.CRS.from_epsg(4326)

        # Only reproject if not already in 4326.
        needs_reproject = "4326" not in crs_wkt and "WGS 84" not in crs_wkt or "Mollweide" in crs_wkt

        if needs_reproject:
            print(f"  Reprojecting from {source_crs.to_epsg() or 'custom CRS'} to EPSG:4326 for MapLibre compatibility")
            ...
            reproject = lambda geom: geom

        fiona_crs = fiona.crs.CRS.from_epsg(4326)
    else:
        # For GPKG, keep the original CRS but fix Mollweide authority.
        if "mollweide" in crs_wkt.lower():
            print("  Detected World Mollweide — writing CRS as ESRI:54009")
            try:
